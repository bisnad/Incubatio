...
def cart2spherical_v2(vector):
    
    # normalize direction
    vector_norm = vector / np.linalg.norm(vector)
    
    azimuth, elevation = spherical_coordinates.cx_cy_cz_to_az_zd(cx=vector_norm[0], cy=vector_norm[1], cz=vector_norm[2])

    return np.array([azimuth, elevation])


vector = np.array([1.0, 0.0, 0.])
print("dir ", vector, "angle ", cart2spherical_v2(vector))
...
def osc_swarm_target_response(address, *args):
    ...
    for light_nr in range(light_count):
       ...
       # target_pos = np.array( [-args[arg_index  + 1] * flock_to_light_pos_scale, -args[arg_index] * flock_to_light_pos_scale, -args[arg_index + 2] * flock_to_light_pos_scale])
        ...
        light_rot_radian = cart2spherical_v2(target_dir)
