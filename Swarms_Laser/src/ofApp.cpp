...
void ofApp::setup()
{
	...
	try
	{
		...
		mocap_warm->addAgents(23); // same as number of mocap joints

		// debug begin
		// change parameter visibility
		int visibleJointIndex = 10; // right hand
		std::vector<dab::flock::Agent*>& mocapAgents = mocap_warm->agents();
		for (int agentI = 0; agentI < 23; ++agentI)
		{
		...
		simulation.com().createReceiver("mocap_receiver", 9002);
		...
		float ring_angle_start = -PI / 2.0; // slight offset of first light
