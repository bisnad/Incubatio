...
example_bvh_file = "mocap/ref_mocap.bvh"
...
def convert_mvn_to_bvh():
    ...
    #print("mvn_rot_world hips tmp ", bvh_rot_world[0])
...
def calc_rot_local():
    ...
    for jI in range(joint_count):
        ...
        if parents[jI] == -1:
            bvh_rot_local[jI] = bvh_rot_world[jI]
...
def calc_rot_pos_world():
    ...
    bvh_pos_world = t_pos_world.numpy()
...
def osc_send():
    ...
    bvh_pos_world_rh = np.copy(bvh_pos_world)

    bvh_pos_world_rh[:, 0] = bvh_pos_world[:, 0] / 100.0
    bvh_pos_world_rh[:, 1] = -bvh_pos_world[:, 2] / 100.0
    bvh_pos_world_rh[:, 2] = bvh_pos_world[:, 1] / 100.0
    
    bvh_rot_world_rh = np.copy(bvh_rot_world)

    bvh_rot_world_rh[:, 1] = bvh_rot_world[:, 1]
    bvh_rot_world_rh[:, 2] = -bvh_rot_world[:, 3]
    bvh_rot_world_rh[:, 3] = bvh_rot_world[:, 2]
