...
ofApp::setupData()
{
	...
	mXSensJointRotEulerData = std::shared_ptr<dab::Data>(new dab::Data("jointRotEuer", 3, jointCount));
	mXSensJointLinVelData = std::shared_ptr<dab::Data>(new dab::Data("jointLinVel", 3, jointCount));
	mXSensJointAngVelData = std::shared_ptr<dab::Data>(new dab::Data("jointAngVel", 3, jointCount));
	mXSensJointLinAccelData = std::shared_ptr<dab::Data>(new dab::Data("jointLinAccel", 3, jointCount));
	mXSensJointAngAccelData = std::shared_ptr<dab::Data>(new dab::Data("jointAngAccel", 3, jointCount));
}
...
ofApp::setupDataProc() throw (dab::Exception)
{
	try
	{
		...
		mXSensJointRotEulerProc = std::shared_ptr<dab::DataProcInput>(new dab::DataProcInput("joint roteuler input", { mXSensJointRotEulerData }));
		mXSensJointLinVelInputProc = std::shared_ptr<dab::DataProcInput>(new dab::DataProcInput("joint linvel input", { mXSensJointLinVelData }));
		mXSensJointAngVelInputProc = std::shared_ptr<dab::DataProcInput>(new dab::DataProcInput("joint angvel input", { mXSensJointAngVelData }));
		mXSensJointLinAccelInputProc = std::shared_ptr<dab::DataProcInput>(new dab::DataProcInput("joint linaccel input", { mXSensJointLinAccelData }));
		mXSensJointAngAccelInputProc = std::shared_ptr<dab::DataProcInput>(new dab::DataProcInput("joint angaccel input", { mXSensJointAngAccelData }));

		// smooth procs
		mXSensJointLinAccelSmoothProc = std::shared_ptr<dab::DataProcLowPass>(new dab::DataProcLowPass("joint lin accel smooth", "jointLinAccel", 0.9));
		mXSensJointAngAccelSmoothProc = std::shared_ptr<dab::DataProcLowPass>(new dab::DataProcLowPass("joint ang accel smooth", "jointAngAccel", 0.9));

		mXSensJointLinJerkSmoothProc = std::shared_ptr<dab::DataProcLowPass>(new dab::DataProcLowPass("joint lin jerk smooth", "jointLinJerk", 0.9));
		mXSensJointAngJerkSmoothProc = std::shared_ptr<dab::DataProcLowPass>(new dab::DataProcLowPass("joint ang jerk smooth", "jointAngJerk", 0.9));

		// derivative procs
		mXSensJointLinJerkProc = std::shared_ptr<dab::DataProcDerivative>(new dab::DataProcDerivative("joint lin jerk calc", "jointLinJerk"));
		mXSensJointAngJerkProc = std::shared_ptr<dab::DataProcDerivative>(new dab::DataProcDerivative("joint lin ang calc", "jointAngJerk"));

		// scalar procs
		mXSensJointLinJerkScalarProc = std::shared_ptr<dab::DataProcScalar>(new dab::DataProcScalar("joint lin jerk scalar calc", "jointLinJerkScalar", dab::DataProcScalar::Max));
		mXSensJointAngJerkScalarProc = std::shared_ptr<dab::DataProcScalar>(new dab::DataProcScalar("joint lin jerk scalar calc", "jointAngJerkScalar", dab::DataProcScalar::Max));
	
		// add procs to pipeline
		mDataProcPipeline->addDataProc(mXSensJointRotEulerProc);
		mDataProcPipeline->addDataProc(mXSensJointLinVelInputProc);
		mDataProcPipeline->addDataProc(mXSensJointAngVelInputProc);
		mDataProcPipeline->addDataProc(mXSensJointLinAccelInputProc);
		mDataProcPipeline->addDataProc(mXSensJointAngAccelInputProc);
		mDataProcPipeline->addDataProc(mXSensJointLinAccelSmoothProc);
		mDataProcPipeline->addDataProc(mXSensJointAngAccelSmoothProc);
		mDataProcPipeline->addDataProc(mXSensJointLinJerkProc);
		mDataProcPipeline->addDataProc(mXSensJointAngJerkProc);
		mDataProcPipeline->addDataProc(mXSensJointLinJerkSmoothProc);
		mDataProcPipeline->addDataProc(mXSensJointAngJerkSmoothProc);
		mDataProcPipeline->addDataProc(mXSensJointLinJerkScalarProc);
		mDataProcPipeline->addDataProc(mXSensJointAngJerkScalarProc);

		// connect procs
		mDataProcPipeline->connect(mXSensJointLinAccelInputProc, mXSensJointLinAccelSmoothProc);
		mDataProcPipeline->connect(mXSensJointLinAccelSmoothProc, mXSensJointLinJerkProc);
		mDataProcPipeline->connect(mXSensJointLinJerkProc, mXSensJointLinJerkSmoothProc);
		mDataProcPipeline->connect(mXSensJointLinJerkSmoothProc, mXSensJointLinJerkScalarProc);
		mDataProcPipeline->connect(mXSensJointAngAccelInputProc, mXSensJointAngAccelSmoothProc);
		mDataProcPipeline->connect(mXSensJointAngAccelSmoothProc, mXSensJointAngJerkProc);
		mDataProcPipeline->connect(mXSensJointAngJerkProc, mXSensJointAngJerkSmoothProc);
		mDataProcPipeline->connect(mXSensJointAngJerkSmoothProc, mXSensJointAngJerkScalarProc);
	}
	catch (dab::Exception& e)
	{
...
ofApp::setupOsc() throw (dab::Exception)
{
	try
	{
		...
		//mDataMessenger->createDataReceiver(mJointLocRotData, "/mocap/joint/locrot", "MocapReceiver");

		mDataMessenger->createSender("MocapProcSender", mOscSendAddress, mOscSendPort);

		mDataMessenger->createDataSender(mJointPosData, "/mocap/joint/pos_world", "MocapProcSender");
		mDataMessenger->createDataSender(mJointPosGlobalToLocalProc->data()[0], "/mocap/joint/locpos", "MocapProcSender");
		mDataMessenger->createDataSender(mJointVelocityProc->data()[0], "/mocap/joint/velocity", "MocapProcSender");
		mDataMessenger->createDataSender(mJointAccelerationProc->data()[0], "/mocap/joint/acceleration", "MocapProcSender");
		mDataMessenger->createDataSender(mJointJerkProc->data()[0], "/mocap/joint/jerk", "MocapProcSender");
		mDataMessenger->createDataSender(mWeightEffortCombineProc->data()[0], "/mocap/joint/weight_effort", "MocapProcSender");
		mDataMessenger->createDataSender(mTimeEffortCombineProc->data()[0], "/mocap/joint/time_effort", "MocapProcSender");
		mDataMessenger->createDataSender(mFlowEffortCombineProc->data()[0], "/mocap/joint/flow_effort", "MocapProcSender");
		mDataMessenger->createDataSender(mSpaceEffortCombineProc->data()[0], "/mocap/joint/space_effort", "MocapProcSender");
		//mDataMessenger->createDataSender(mJointRotData, "/mocap/joint/rot_quat", "MocapProcSender");
		mDataMessenger->createDataSender(mJointRotData, "/mocap/joint/rot_world", "MocapProcSender");

		// additional senders for quaternion derivatives for debug reasons
		mDataMessenger->createDataSender(mJointRotQuatVelocityProc->data()[0], "/mocap/joint/rot_velocity_quat", "MocapProcSender");
		mDataMessenger->createDataSender(mJointRotQuatAccelerationProc->data()[0], "/mocap/joint/rot_acceleration_quat", "MocapProcSender");
		mDataMessenger->createDataSender(mJointRotQuatJerkProc->data()[0], "/mocap/joint/rot_jerk_quat", "MocapProcSender");
		
		mDataMessenger->createDataSender(mJointRotGlobalToLocalProc->data()[0], "/mocap/joint/locrot_quat", "MocapProcSender");
		...
		mDataMessenger->createDataSender(mJointRotQuatEulerProc->data()[0], "/mocap/joint/rot", "MocapProcSender");
		mDataMessenger->createDataSender(mJointRotQuatVelocityEulerProc->data()[0], "/mocap/joint/rot_velocity", "MocapProcSender");
		mDataMessenger->createDataSender(mJointRotQuatAccelerationEulerProc->data()[0], "/mocap/joint/rot_acceleration", "MocapProcSender");
		mDataMessenger->createDataSender(mJointRotQuatJerkEulerProc->data()[0], "/mocap/joint/rot_jerk", "MocapProcSender");
		mDataMessenger->createDataSender(mRotWeightEffortCombineProc->data()[0], "/mocap/joint/rot_weight_effort", "MocapProcSender");
		mDataMessenger->createDataSender(mRotTimeEffortCombineProc->data()[0], "/mocap/joint/rot_time_effort", "MocapProcSender");
		mDataMessenger->createDataSender(mRotFlowEffortCombineProc->data()[0], "/mocap/joint/rot_flow_effort", "MocapProcSender");
		mDataMessenger->createDataSender(mRotSpaceEffortCombineProc->data()[0], "/mocap/joint/rot_space_effort", "MocapProcSender");

		mDataMessenger->setDataSenderActive(mJointPosData, true);
		mDataMessenger->setDataSenderActive(mJointPosGlobalToLocalProc->data()[0], false);
		mDataMessenger->setDataSenderActive(mJointVelocityProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mJointAccelerationProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mJointJerkProc->data()[0], true);
		...
		mDataMessenger->setDataSenderActive(mJointRotData, true);
		...
		mDataMessenger->setDataSenderActive(mJointRotQuatEulerProc->data()[0], true);
		...
		// new procs for calculating linear and angular jerk from xsens kinematics data
		mDataMessenger->createDataReceiver(mXSensJointRotEulerData, "/mocap/joint/rot_world_euler", "MocapReceiver");
		mDataMessenger->createDataReceiver(mXSensJointLinVelData, "/mocap/joint/lin_vel", "MocapReceiver");
		mDataMessenger->createDataReceiver(mXSensJointAngVelData, "/mocap/joint/rot_vel", "MocapReceiver");
		mDataMessenger->createDataReceiver(mXSensJointLinAccelData, "/mocap/joint/lin_acc", "MocapReceiver");
		mDataMessenger->createDataReceiver(mXSensJointAngAccelData, "/mocap/joint/rot_acc", "MocapReceiver");

		mDataMessenger->createDataSender(mXSensJointRotEulerProc->data()[0], "/mocap/joint/rot_world_euler", "MocapProcSender");
		mDataMessenger->createDataSender(mXSensJointLinVelInputProc->data()[0], "/mocap/joint/lin_vel", "MocapProcSender");
		mDataMessenger->createDataSender(mXSensJointAngVelInputProc->data()[0], "/mocap/joint/rot_vel", "MocapProcSender");
		mDataMessenger->createDataSender(mXSensJointLinAccelInputProc->data()[0], "/mocap/joint/lin_acc", "MocapProcSender");
		mDataMessenger->createDataSender(mXSensJointAngAccelInputProc->data()[0], "/mocap/joint/rot_acc", "MocapProcSender");
		mDataMessenger->createDataSender(mXSensJointLinJerkScalarProc->data()[0], "/mocap/joint/lin_jerk", "MocapProcSender");
		mDataMessenger->createDataSender(mXSensJointAngJerkScalarProc->data()[0], "/mocap/joint/ang_jerk", "MocapProcSender");

		mDataMessenger->setDataSenderActive(mXSensJointRotEulerProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mXSensJointLinVelInputProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mXSensJointAngVelInputProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mXSensJointLinAccelInputProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mXSensJointAngAccelInputProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mXSensJointLinJerkScalarProc->data()[0], true);
		mDataMessenger->setDataSenderActive(mXSensJointAngJerkScalarProc->data()[0], true);
...
ofApp::setupGraphics()
{
	...
	// new procs for calculating linear and angular jerk from xsens kinematics data
	mDataPlots[16] = std::make_shared<dab::DataPlot>("Rot Euler", mXSensJointRotEulerProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[16]->setDataRange({ -180, -180, -180 }, { 180, 180, 180 });

	mDataPlots[17] = std::make_shared<dab::DataPlot>("Lin Vel", mXSensJointLinVelInputProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[17]->setDataRange({ -4.0, -4.0, -4.0 }, { 4.0, 4.0, 4.0 });

	mDataPlots[18] = std::make_shared<dab::DataPlot>("Ang Vel", mXSensJointAngVelInputProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[18]->setDataRange({ -10.0, -10.0, -10.0 }, { 10.0, 10.0, 10.0 });

	mDataPlots[19] = std::make_shared<dab::DataPlot>("Lin Accel", mXSensJointLinAccelInputProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[19]->setDataRange({ -10.0, -10.0, -10.0 }, { 10.0, 10.0, 10.0 });

	mDataPlots[20] = std::make_shared<dab::DataPlot>("Ang Accel", mXSensJointAngAccelInputProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[20]->setDataRange({ -20.0, -20.0, -20.0 }, { 20.0, 20.0, 20.0 });

	mDataPlots[21] = std::make_shared<dab::DataPlot>("Lin Jerk", mXSensJointLinJerkScalarProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[21]->setDataRange({ 0.0 }, { 100.0 });

	mDataPlots[22] = std::make_shared<dab::DataPlot>("Ang Jerk", mXSensJointAngJerkScalarProc->data()[0], plotHistoryLength, plotPos, plotSize);
	mDataPlots[22]->setDataRange({ 0.0 }, { 500.0 });
