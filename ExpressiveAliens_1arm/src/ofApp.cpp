...
ofApp::setup()
{
	...
	try
	{
		...
		urdfImporter.loadURDF(ofToDataPath("3d_models/base_links_v5_v3/robot.urdf")); // even more segments in one arm (23 joints to be precise)
...
ofApp::setupPhysics()
{
	...
	try
	{
		...
		*/
		...
		glm::vec3 bodyPos(0.0, 0.0, 0.0);
		//glm::quat bodyRot(glm::vec3(PI / 2.0, 0.0, 0.0));
		glm::quat bodyRot(glm::vec3(PI / 2.0, PI / 2.0, 0.0));
